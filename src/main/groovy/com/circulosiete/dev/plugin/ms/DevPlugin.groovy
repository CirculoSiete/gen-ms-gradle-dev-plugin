...
class DevPlugin implements Plugin<Project> {
  ...
  void apply(Project project) {
    ...
    project.task([type: com.bmuschko.gradle.docker.tasks.image.Dockerfile, dependsOn: 'setupMs'], 'createDockerfile') {
      ...
      entryPoint 'java', "-Djava.awt.headless=true", "-Duser.timezone=${project.ext.jvmTimeZone}", "-Xms${theJvmMemory}", "-Xmx${theJvmMemory}", "-XX:ParallelGCThreads=2", "-XX:CICompilerCount=2", '-jar', theMicroserviceJar, 'server', '/config/config.yaml'
